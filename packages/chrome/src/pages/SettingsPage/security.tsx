  ...
  return (
    <div className={styles["security-setting-container"]}>
      <div className="flex justify-end items-center h-14">
        <div className="setting-cancel" onClick={() => navigate('/settings')}></div>
      </div>
      ...
      <div>
        <div className={styles["secutity-card"]}>
          ...
          <div className={styles["security-desc"]}>change your wallet login password</div>
          <Button onClick={() => navigate('password', {
            state: {
              hideApplayout: true,
            }
          })} className="mb-8">Update Password</Button>
        </div>
        <div className={styles["security-line"]} />
        <div className={styles["secutity-card"]}>
          <div className={styles["security-title"]}>Recovery Phrases</div>
          <div className={classnames(styles["security-desc"], styles['warning'])}>
          ...
          <Modal 
            ...
            onOpenChange={async () => {
              const rawPhrases = await coreApi.wallet.revealMnemonic(context.walletId, context.token);
          ...
          >
            <div className={styles["security-modal-content"]}>
              ...
              <div key={p} className={classnames("flex items-center", styles['phrase-container'])}>
                <span className="inline-block text-gray-300 text-right select-none">{`${index + 1 + 0}`}</span>
        ...
        <div className={styles["secutity-card"]}>
          ...
          <Modal 
            ...
            onOpenChange={async () => {
              const privateKey = await coreApi.wallet.revealPrivate(context.walletId, context.token);
...
function PasswordSetting() {
  ...
  return (
    <SetPassword onNext={handleSetPassword} />
  )
}
